...
const MainScreen = ({
    ...
    const modes = [
        ...
        {id: "deep_mode", name: "Deep Focused", desc: "Strict focus mode", icon: <Sprout className='w-8 h-8'/> , color: 'bg-pomo-yellow'},
    ...
    <>
        ...
        <div className='grid grid-cols-2 mt-8 gap-4'>
            {modes.map((item) => (
            <div key={item.id} onClick={() => handleModeBtn(item.id)} className={cn('rounded-xl cursor-pointer items-center justify-center hover:scale-105 gap-2 transition-all ease-in-out border-pomo-text border-4 flex flex-col p-4', item.color)}>
